...
test.describe('User managment app tests', async () => {
  const clearUser = async (id: number, request: APIRequestContext): Promise<void> => {
    await request.delete(`http://localhost:3001/users/${id}`)
  }

  test('TL-14-1 create user test', async ({ request }) => {
    const creationResponse = await request.post('http://localhost:3001/users')
  ...
  test('TL-14-2 find user test', async ({ request }) => {
    const creationResponse = await request.post('http://localhost:3001/users')
    const createdUser = UserDTO.serializeResponse(await creationResponse.json())
    const searchResponse = await request.get(`http://localhost:3001/users/${createdUser.id}`)
  ...
  test('TL-14-3 delete user test', async ({ request }) => {
    const creationResponse = await request.post('http://localhost:3001/users')
    const createdUser = UserDTO.serializeResponse(await creationResponse.json())
    const deleteResponse = await request.delete(`http://localhost:3001/users/${createdUser.id}`)
  ...
  test('TL-14-4 get all users test', async ({ request }) => {
    ...
    await request.post('http://localhost:3001/users')
    await request.post('http://localhost:3001/users')
    const allUsersResponse = await request.get('http://localhost:3001/users')
